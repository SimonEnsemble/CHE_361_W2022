...
# ╔═╡ 0e10841a-7a7b-11ec-105f-ff1f2804b08e
using DifferentialEquations, CairoMakie, DataFrames

# ╔═╡ 33de5565-44a3-4338-b213-31cd103bf4a2
md"# solving ODEs via `DifferentialEquations.jl`

see [docs](https://diffeq.sciml.ai/stable/tutorials/ode_example/).
"

# ╔═╡ b541f157-5777-4140-80c4-4dc545f7148f
set_theme!(theme_light()); update_theme!(linewidth=4, fontsize=20)

# ╔═╡ ea93badf-130e-4a68-924c-ad27606717bf
md"
the liquid level $h=h(t)$ [m] in a cylindrical tank changes with time $t$ [min] and is governed by:
```math
\begin{equation}
...
```
with parameters:
*  $A$ [m²]: cross-sectional area of the cylinder
*  $R$ [m²/min]: resistance to flow out of tank
and input:
*  $q_i=q_i(t)$ [m³/min]: inlet flow rate of liquid
...
# ╔═╡ c223f016-2c3b-46e8-90ba-b915003084fd
md"🐒 define params"

# ╔═╡ 85515d0f-bee7-4a6b-bdcc-1958fbabff35
begin
	A = 1.0 # m²
	R = 2.0 # m² / min
end

# ╔═╡ e94cb45b-dbe4-4827-8ee8-43cbccc59fb2
md"🐒 define input $q_i(t)$"

# ╔═╡ 2513d413-1194-4b3d-9d47-7703aab79f66
function qᵢ(t)
	if t < 15.0
		return 0.2
	else
...
# ╔═╡ 44d58cfd-f1b1-4fda-9278-6aa9d109874e
md"🐒 define right-hand-side of ODE."

# ╔═╡ eafd3d50-71d3-4d87-adbd-fef03d9b18e0
#=
view ODE as dh/dt = f(h, t; θ)
this function characterizes the ODE, then.

* h: output (state of system)
...
function f(h, θ, t)
	...
	return (qᵢ(t) - h / R) / A
	# return (qᵢ(t) - h / θ[2]) / θ[1] # alternative way
end

# ╔═╡ 50aceb11-80a9-419f-b7f5-988c6ca899d8
md"🐒 time span over which we wish to solve the ODE."

# ╔═╡ 11079683-3132-4a06-a2e5-1ec1bd18e5eb
time_span = (0.0, 40.0) # min

# ╔═╡ 98a83c18-5456-4eef-ab48-b587f01012c5
md"🐒 initial condition"

# ╔═╡ 8696549f-4ab0-4626-a1ff-db6bb257cda1
h₀ = 0.0

# ╔═╡ 4be94d9a-a8e1-459a-83dd-fe57dedbf29a
md"🐒 define and solve ODE"

# ╔═╡ 37a8a753-c229-4a80-bdf8-63b63399913c
begin
	# vector of params
	θ = [A, R]
	
	prob = ODEProblem(f, 0.01, time_span, θ, 
		              saveat=0.25, d_discontinuity=[15.0])
	
	h_data = DataFrame(solve(prob))
end

# ╔═╡ 7bfc3977-a4a9-4aba-9360-2a5cad409a0b
md"🐒 here's the list of times, tᵢ's, and a list of corresponding liquid levels, hᵢ's."

# ╔═╡ 1e7c608a-15a1-4495-b46f-fdddbc023b6f
h_data[:, "timestamp"]

# ╔═╡ bb79a0c5-6424-4af4-b6b7-48db8e4b2b9c
h_data[:, "value"]

# ╔═╡ 0a6891cc-55aa-4b07-8565-12b7b7d2c9ea
begin
	fig = Figure()
	
	ax1  = Axis(fig[1, 1], 
		        xlabel="time, t [min]", 
		        ylabel="inlet flow rate\nqᵢ(t) [m³/min]",
				title="input"
	)
	ax2  = Axis(fig[2, 1], 
		        xlabel="time, t [min]", 
	...
	lines!(ax1, t, qᵢ.(t), color="seagreen")

	# plot output
	lines!(ax2, h_data[:, "timestamp"], h_data[:, "value"], color="red")
	hlines!(ax2, maximum(h_data[:, "value"]), 
		   linestyle=:dash, color="gray", linewidth=1)
	
	fig
end

# ╔═╡ 0aac8543-888e-4e79-b56d-0dec4dd8500f
md"🐒 find when, during the emptying phase, the liquid level drops below 0.1 m."

# ╔═╡ ffaa63c3-2e6c-4fff-b7c1-6d2489089e97
h_data[findlast(h_data[:, "value"] .> 0.1), "timestamp"]
