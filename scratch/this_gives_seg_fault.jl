...
set_theme!(theme_light()); update_theme!(linewidth=4, fontsize=20)

# ╔═╡ 04ab13f8-0698-465a-882c-ac6128cd810b
md"
# solving ODEs and simulating the saline solution facility
...
the following differential equation is a dynamic model for a process to produce saline solution (see lecture notes and hw1) when accounting for the dependence of the density of the solution on the salt concentration:
```math
\begin{equation}
    V \frac{dc}{dt} = w - c \left[q_w-\frac{\alpha-1}{\rho_w} w \right]
\end{equation}
```

*  $V$ [L]: volume of liquid in the tank (constant as a consequence of the overflow line)
*  $c$ [g salt/L solution]: concentration of salt in the outlet stream
*  $q_w$ [L water/ s]: incoming volumetric flow rate of pure water
*  $w$ [g salt / s]: incoming gravimetric flow rate of solid salt granules
*  $\alpha$ [g solution /g salt]: parameter characterizing the density of saline solution as a function of salt concentration (see hw1)
"

# ╔═╡ 82c8f328-0243-4108-80a3-07dce9fe7c1f
html"<img src=\"https://raw.githubusercontent.com/SimonEnsemble/CHE_361_W2022/main/studios/salt_water_mixer.png\" width=300>"

# ╔═╡ 5fa98241-6651-4363-99bf-20604f079ad3
md"                                                                                   
**task**: simulate the dynamics of the saline solution facility---i.e., numerically solve the ODE for the salt concentration as a function of time, $c(t)$---under a disturbance in the salt input $w(t)$.
                                                                                    
take the parameters as:                                                                          
*  $V=10$ L                                         
*  $\alpha=0.64$                                                                     
*  $\rho_w=1000$ g/L 
and treat the water input as constant:
*  $q_w=0.1$ L/s     

in this scenario, mathematically:
* the *input* is $w=w(t)$
* the *output* (which responds to changes in the input) is $c=c(t)$

!!! example \"customer standards\"
	the customer (a hospital) wishes for saline solution with a salt concentration of $\bar{c}=0.1$ g/L. however, they will accept (buy) saline solution that is in the range $c\in[0.095, 0.105]$ g/L, so within 0.005 g/L of the target $\bar{c}$.
                                                                                            
🐝 define the variables `V`, `qw`, `α`, `ρw` as the appropriate constants in the cell below.
"  
...
🐝 to produce saline solution that meets the customer specifications, we wish to achieve and maintain an output salt concentration of $\bar{c}$ at steady state conditions. given the value of the constant incoming flow rate of water, $q_w$, compute the steady state value $\bar{w}$ [g/s] of the incoming salt flow rate, $w$, and assign it as a variable `w̄`.
                                                                                    
!!! hint
    you probably need to use `fzero`.                                                 
" 
...
# ╔═╡ 50fd502a-9a36-462a-8c5d-bcb69555c890
fzero(h, 0.0001)

# ╔═╡ b9d9a8b6-e239-4c22-9d41-3f63be0165e7
w̄ = fzero(h, 0.0001)
...
the process is initially ($t=0$) operating at steady state: (i) with inlet salt flow rate$\bar{w}$ and (ii) producing saline solution with salt concentration $\bar{c}$.
...
🐝 model this input $w=w(t)$ as a rectangular pulse---a piecewise flat function. sketch $w(t)$ and write it mathematically as a piecewise function.
...
!!! hint
	you will need to use an \"if\" `if` and \"or\" `||` statement. see Julia docs on [control flow](https://docs.julialang.org/en/v1/manual/control-flow/). to exemplify:
	```julia
	# input: temperature [°C]
	# output: is it a comfortable temperature?
	function comfortable(temperature)
...
# ╔═╡ e65ca3ba-97e7-4dfa-a616-2e256c89fc74
w(1.0) # should return w̄
...
begin
	t = range(0.0, 600.0, length = 300)
	fig = Figure()
	ax1 = Axis(fig[1,1],
				xlabel= "time, t [sec]", 
				ylabel = "mass flow rate of salt, w [g]",
...
md"🐝 before you simulate, sketch out what you anticipate the output salt concentration $c(t)$ to look like in response to this rectangular pulse input $w(t)$.
...
# ╔═╡ 5ac88c1d-4808-4773-8746-826343551116
function happy(c,θ,t)
	V, a, p = θ
	return w(t)-c*(q-((a-1)/p)*w(t))/V
end


# ╔═╡ 7ad1356d-a87d-40a0-b5bb-bc9695e421fd
c₀ = 0.0 # initial concentration

# ╔═╡ 743a9c53-5405-410d-8df8-232cf86ba60e
time_span = (0.0, 600.0)
...
begin 
	...
	prob = ODEProblem(happy, 0.01, time_span, θ,  
		              saveat=0.25, d_discontinuities=[25.0, 100.0])

	h_data = DataFrame(solve(prob))
end
...
🐝 see the green box above on customer standards. owing to the conveyor belt outage, for what time interval is the process producing saline solution that does not meet customer standards?
...
```
to highlight, on the same plot, the time interval over which the saline solution cannot be sold. see the [`Makie.jl` docs](https://makie.juliaplots.org/dev/examples/plotting_functions/hvspan/).
"
...
git-tree-sha1 = "79f6198e29e774079ccd652ef3701e69c2a0248d"
