...
md"
!!! note \"resources\"
	* my `Optim.jl` in-class demo
	* my notes on the dynamics of the heat transfer to/from a thermistor
...
_variables and parameters_:
* time, $t$ [s]
* input, $T_e=T_e(t)$ [°C]: temperature of the external medium (air, liquid, or gas) surrounding and in contact with the thermistor.
* output, $T=T(t)$ [°C]: temperature of the thermistor
  
_dynamic model_ in the time domain:
```math
\begin{equation}
...
*  $U$ [J/(s⋅m²⋅°C)]: heat transfer coefficient
*  $A$ [m²]: surface area
*  $C$ [J/°C]: thermal capacity
...
\end{equation}
```
with (lumped) parameter:
*  $\tau:=C/(UA)$ [s]: time constant

and $\mathcal{T}^*(s) := \mathcal{L}[T^*(t)]$ and $\mathcal{T}_e^*(s) := \mathcal{L}[T_e^*(t)]$.

_objective_: empircally determine the time constant $\tau$ of the thermistor from response data. this contrasts to conducting three separate experiments to identify each of the physical parameters $C$, $U$, and $A$ of the thermistor separately.
"
...
md"
## the input-output experiment on the thermistor

Grant, an LA last year, grabbed and held the thermistor for a long time; the temperature of the thermistor eventually reached thermal equilibrium with Grant's hand and adopted his body temperature, $T_{Grant}$ [°C]. 
...
owing to a temperature differential between the (hotter) thermistor and (colder) air, heat was transferred from the thermistor to the air. as a consequence, the temperature of the thermistor dropped and approached ambient temperature.
...
md"
🐜 visualize the thermistor reponse time series data (i.e., the response $T(t)$) with a scatter plot. label your axes and include appropriate units. 

!!! hint
	to make the points easier to see, make markers hollow via passing `color=(\"white\", 0.0), strokecolor=\"black\", strokewidth=1` to `scatter`.
"

# ╔═╡ d95d456e-de7c-4eb9-bf64-a72a9ce0fe6d


# ╔═╡ 51b889ad-bd2e-4845-b0c0-5f89d9af2806
md"### calculate $T_{Grant}$ and $T_{air}$"

# ╔═╡ 95d39791-b5c2-4377-a698-aeb3cbf3aa5b
md"🐜 calculate the temperature of Grant's finger and assign it as a variable `T_Grant` by taking the average (`mean`) of the temperature reading when $t<0$. during this time, Grant was still holding onto the thermistor. so, we are assuming the thermistor was in thermal equilibrium with Grant's hand.

!!! hint
	you may find the `filter` function useful. see [here](https://dataframes.juliadata.org/stable/lib/functions/#Base.filter).
"

# ╔═╡ 9e008e05-affb-495c-8e80-3c3e93f443fa


# ╔═╡ a778f497-5331-4869-9c75-83f6ee5fe2f5
md"🐜 via a similar procedure, calculate the ambient temperature of the air and assign it a variable `T_air` by taking the average of the temperature for $t>40$ s after which we assume the thermistor has reached thermal equilibrium with the ambient air."

# ╔═╡ 72350a4c-45ac-4ba2-a78e-aa6df3c6e0bf


# ╔═╡ 9a0d26a6-10c1-4908-ae26-95a633e906fc
md"
🐜 draw a new visualization of the $T(t)$ time series data, this time including two horizontal, dashed lines that indicate `T_Grant` and `T_air`. give them `label`s and include a legend to indicate what each curve/line represents.

you should see the temperature of the thermistor go from `T_Grant` to `T_air`.

!!! hint
	the `hlines!` function may be helpful. see [here](https://makie.juliaplots.org/stable/examples/plotting_functions/hvlines/).

!!! hint
	change the position of the legend by passing `position`. see [here](https://makie.juliaplots.org/stable/examples/layoutables/legend/index.html#legend_inside_an_axis)

"

# ╔═╡ ee58ae17-d46d-4a7b-838f-02856afb1155
...
**the objective** is to use the data to empirically identify the time constant $\tau$ of heat transfer to/from this thermistor.
...
md"🐜 given a guess for the time constant $\tau$, write a function `T(t, τ)` that returns the temperature according to the first order model for $T(t)$ and specified time constant $\tau$. 
...
md"🐜 write a function `loss(τ)` that computes the loss (sum of square residuals between the predicted temperature and actual temperature) for a given guess for `τ`. we seek to minimize the loss to determine the time constant `τ`. "
...
md"🐜 use the `optimize` function in `Optim.jl` to minimize the loss function you wrote by tuning $\tau$. assign the optimum value of $\tau$ as a variable `τ_opt`. what is the optimal value of $\tau$ that best fits the data? what are the units of $\tau$?
...
# ╔═╡ 41fdb0e3-8fbf-457c-8c98-09c3ffb32430
md"
### the utility of empirical models

now that we have an empirical model for heat transfer to/from the thermistor (i.e., now that we've identified the time constant $\tau$ of the thermistor), we can predict the response of the thermistor to inputs that differ from the input in this experiment. this is the real value of having an empirical model!

🐜 
suppose the external air temperature varies sinusoidally according to:
```math
\begin{equation}
T_e^*(t)=\sin(10t) \text{ }^\circ \text{C}
\end{equation}
```
with the $10$ in units of seconds. using `Controlz.jl`, simulate and plot the response $T^*(t)$ to a sinusoidal input in for two minutes.
"

# ╔═╡ 92a55b27-e42c-4c40-9d13-381904ecf7b0


# ╔═╡ 58db5d02-e114-44c6-b528-02ad4980a795


# ╔═╡ b79e7344-706e-437c-9a23-00c9dd7fae10
md"
🐜 the amplitude of the input was one deg. C. but, the output amplitude here is not one despite the gain of the transfer function being one. 
this is quite concerning from a temperature sensing standpoint! why is this concerning, and why does the output amplitude not match the input amplitude?
"

# ╔═╡ 8f7fd69d-76e3-4589-9fd4-2a1f32fdb0c8
md"
...
manifest_format = "2.0"
project_hash = "d8c5361436e7d75722302fb141b8f0637f38a8c0"
...
git-tree-sha1 = "79c4e8ac46509204e301ea13a7c0ff56f233d795"
...
git-tree-sha1 = "ef5af2aebac86f0b6ba127e09047afcb1ae2492d"
