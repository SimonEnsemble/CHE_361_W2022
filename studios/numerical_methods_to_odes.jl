...
* flow rate out of the tank is driven by hydrostatic pressure and is related to the liquid level as $c\sqrt{h}$ [m$^3$/s]
...
md"🐸 code up the function `f(h)`, defined in relation to your dynamic model for $h=h(t)$ via:
...
md"
🐸 to set up our finite difference approximation, define the time step `Δt` to be 0.05 s.
"
...
md"🐸 we wish to simulate the model in the time interval $t \in [0, 3]$ min. the variable `tₙ` is defined to be the final time **in seconds**, after we take the `n` time steps to arrive at it."

# ╔═╡ 7993b7e5-3e02-4863-a610-3471008afcb8


# ╔═╡ 3295e2fe-baea-4766-9210-f21f1ec435c1
md"🐸 compute and define `n`---the number of time steps we wish to take---in terms of `Δt` and `tₙ` so that we can change the time step later and have `n` automatically update, accordingly. 
...
md"🐸 discretize time by constructing an array `t` with the time points $t_i$. i.e. element $i$ of the array `t` should be equal to $t_i= \Delta t (i-1)$ for $i \in \{1, 2, ..., n+1\}$. we go up to $n+1$ because the first element of `t` should be zero. the last entry of the `t` array should be `tₙ`.

!!! hint
	preallocate an array of zeros, loop through each entry and overwrite entries with the appropriate values.
"
...
md"
🐸 we will store the approximations to the solution, $h_i \approx h(t_i)$'s, in an array `h`. pre-allocate this array with zeros, of the appropriate size to correspond to `t`, so $n+1$ elements. we'll overwrite these zeros later.
...
md"
🐸 implement the forward Euler method to approximate the solution to this ODE and fill in the rest of `h`. march ahead in time for `n` steps, via a `for` loop. use all of the following that you defined earlier:
* the `h` array
* the `t` array
* `Δt`
* `f(h)`
...
!!! note
	you should see a `DomainError`. the domain error comes from passing a negative value of $h$ into a square root function contained in $f(h)$. this happens at the time point when the tank is fully emptied and the dynamics are \"over\", but there is a little bit of error emanating from the finite difference approximation. to fix it, *when the tank is finally empty* inside the for loop, stop the time stepping since we know $h(t)=0$ for this time point and beyond.
"
...
!!! hint
	if you see a little blip in your plot at the point where the tank empties of liquid, you need to assign `h[i]` at that time point to be zero during your forward Euler routine.
"
...
md"
🐸 does the liquid level drop faster early in the emptying process or later?
why is that? think about *both* the tank geometry and the hydrostatic pressure at the bottom of the tank that is driving flow out of it. they are _competing_ effect, with one dominating!
...
md"🐸 use the analytical solution to determine the time $t_e$ at which the tank empties, i.e. the time that the liquid level first reaches zero:
```math
\begin{equation}
