...
in this studio, we'll examine how the poles, damping coefficient $\xi$, and time constant $\tau$ affect the step response of a SO system by comparing the responses of pairs of two SO systems.
...
🌵 for each transfer function, on pencil and paper, *without using `Controlz.jl`* (but okay to use Julia e.g. `Polynomials.jl` for calculations), determine the:
* gain $K$
* time constant $\tau$
...
function viz_poles(poles_a::Vector, poles_b::Vector, 
	...
	scatter!(real.(poles_a), imag.(poles_a), marker=:x, markersize=18, label=label_a)
	scatter!(real.(poles_b), imag.(poles_b), marker=:x, markersize=18, label=label_b)
...
_system 1_, described by $G_1(s)$:

_system 2_, described by $G_2(s)$:
"
...
* the time scale of the response
  * the time for the output to _first reach_, say, 95% of the new steady state
  * the time to _settle_ at the new steady state

to:
* the location of the poles in the complex plane
* the values of $\tau$ and $\xi$
"
...
md"🌵 use the `viz_poles` function I've provided to plot the poles of the two transfer functions _on the same plot_, but with different colors. the labels are needed to indicate which markers correspond to which transfer function."
...
_system 3_, described by $G_3(s)$:

_system 4_, described by $G_4(s)$:
"
...
md"🌵 connect:
* the presence or absence of oscillations
* the time scale of the response, defined as here, the time for the output to _first reach_, say, 95% of the new steady state, as opposed to _settling_ at the new steady state
...
_ambitious Beavers_: use `ColorSchemes.jl` to intelligently color the curve according to the value of $\xi$ with a colormap.
!!! hint
...
manifest_format = "2.0"
project_hash = "2fe8cd507943e78636d53c0e00328e769e1dc09d"
...
git-tree-sha1 = "18246d784208e9e724d4449dec152b99b0741ec3"
