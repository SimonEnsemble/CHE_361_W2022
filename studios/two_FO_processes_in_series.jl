...
a drug is ingested. it absorbs into the blood through the GI tract. it decays in the blood. this can be viewed as two first order processes in series. the dynamic model for the dynamics of the drug concentration in the human body (GI tract, blood) is:
...
md"🍕 use `Controlz.jl` to contruct the transfer function that relates changes in (input) $I(s)$ to changes in (output) $G(s)$. 
...
md"🍕 now contruct the transfer function that relates changes in (input) $G(s)$ to changes in (output) $B(s)$."

# ╔═╡ 21b6a29f-80e4-4ea1-9655-1e3d62817447


# ╔═╡ 93f53302-743b-49ff-a057-2b4cf651ab0b
md"🍕 contruct the \"composite\"/\"overall\" transfer function that relates changes in (input) $I(s)$ to changes in (output) $B(s)$.

!!! hint
	you can do algebra on transfer functions so shouldn't construct this from scratch. rather, use your two previously defined transfer functions. see [here](https://simonensemble.github.io/Controlz.jl/dev/tfs/#transfer-function-algebra). this might be a good way to check your algebra in your write-up, too.
"
...
md"🍕 swallowing a pill of $A$ mg at $t=0$ can be modeled as an impulse input $i(t)=A \delta(t)$, with $\delta(t)$ the Dirac delta function. 
...
2. how many hours after the first pill is swallowed does the effectiveness of the drug wear off (assuming no more pills are taken)?
...
manifest_format = "2.0"
project_hash = "3f778db754b4fc673de3933f928c88f3b9ba31d1"
